module.exports = function(less, options) {
	...
	return {
		...
		versioned: function( filename ) {
			...
			var file, filepath, root, version, hash, path, url, sourcefile;
			try {
				if (filename.currentFileInfo && filename.currentFileInfo.filename) { //old LESS compiler
					//console.log(JSON.stringify(filename.currentFileInfo));
					sourcefile = filename.currentFileInfo.filename;
					root = less.path.dirname(filename.currentFileInfo.filename) + '/';
				}
				else { //newer LESS
					sourcefile = filename._fileInfo.filename;
					root = filename._fileInfo.currentDirectory;

					//console.log(JSON.stringify(filename._fileInfo));
					if (sourcefile !== filename._fileInfo.rootFilename) {
						sourcefile += ' included from ' + filename._fileInfo.rootFilename;
					}
				}
				...
				if ('/' === filename.value[0]) { //absolute file name - look for it in WWW root
					...
					if (options.verbose) {
						console.log('Looking for file ' + filename.value + ' inside ' + options.root + ' folder. Referenced from ' + sourcefile);
					}
				}
					...
					if (options.verbose) {
						console.log('Looking for file ' + filename.value + ' inside ' + root + ' folder. Referenced from ' + sourcefile);
					}
				}
			...
			try {
				if (filepath in versionCache) {
					...
					if (file && file.isFile()) {
						...
						console.warn('File "' + filepath + '" referenced from ' + sourcefile + ' not found or is not a file.');
						hash = '00000000';
					}
				...
				console.error('Failed to process function versioned(' + filename.quote + filename.value + filename.quote + ') with error "' + e + '" in file ' + sourcefile);
