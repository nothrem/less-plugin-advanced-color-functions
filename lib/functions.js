		...
		versioned: function( filename ) {
			...
			try {
				...
				if (file && file.isFile()) {
					...
					if (options.verbose) {
						console.log('File ' + filepath + ' referenced in ' + filename.currentFileInfo.filename + ' has version hash ' + hash);
					}
				}
				else {
				...
				var path = new less.tree.Quoted(filename.quote, filename.value + '?' + hash, filename.escaped, filename.index, filename.currentFileInfo);
				var URL = new less.tree.URL(path, filename.index, filename.currentFileInfo, false);
				return URL;
			}
			catch (e) {
